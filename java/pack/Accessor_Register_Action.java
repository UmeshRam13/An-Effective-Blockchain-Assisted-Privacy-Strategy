...
public class Accessor_Register_Action extends HttpServlet {
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            PreparedStatement p=con.prepareStatement("insert into accessor_register(username,password,mobile,mail,dob,gender,pic)values('"+m.get(0)+"','"+m.get(1)+"','"+m.get(2)+"','"+m.get(3)+"','"+m.get(4)+"','"+m.get(5)+"','"+ms.get(0)+"')");
            p.executeUpdate();
            System.out.println(p);
            out.println("<script>"
                        +"alert('Regsitered Sucessfully')"
                        +"</script>");
                RequestDispatcher rd=request.getRequestDispatcher("Data_Owner.jsp");
